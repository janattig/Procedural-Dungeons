...
execute store result score %target_tp_dimension pd_dimensions run data get storage pd_dimensions:regions resetting[0][4] 1
...
data modify entity @e[type=marker, tag=pd_dim_reset_marker_tmp, limit=1] Pos set from storage pd_dimensions:regions resetting[0]

# load the region size to a temporary scoreboard value
scoreboard players reset %current_reset_region_size pd_dimensions
execute store result score %current_reset_region_size pd_dimensions run data get storage pd_dimensions:regions resetting[0][3] 1
